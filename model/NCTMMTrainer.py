...
from src.modules.MixtureGraphProbs import MixtureGraphProbs


class NCTMMTrainer(RhinoTrainer):
    def __init__(self,
        ...
        self.mixture_selection = MixtureGraphProbs(num_graphs=self.num_graphs,
    ...
    def compute_loss(self, X_history, x_current, idx):
        ...
        mixture_probs = self.mixture_selection.get_probs(idx)#shape: (num_graphs, batch)
        # sample G from variational distributions
    ...
    def compute_loss_terms(self,
        ...
        if self.likelihood_loss == 'mse':
            ...
            likelihood_term = mixture_probs.transpose(
                0, -1) * component_wise_likelihood
            ...
            component_wise_likelihood = log_prob * \
                                        mixture_probs.transpose(0, -1)
            # component_wise_likelihood = log_prob
            likelihood_term = - \
                torch.sum(torch.mean(component_wise_likelihood, dim=0))

        mixture_index = torch.argmax(mixture_probs, dim=0)
        mse_loss = self.compute_mse(
            x_current[:, mixture_index], x_pred[:, mixture_index])
        mape_loss = self.compute_mape(
            x_current[:, mixture_index], x_pred[:, mixture_index])
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.use_indices is not None:
            ...
            self.mixture_selection.manual_set_mixture_indices(
                idx, self.use_indices[idx])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss_terms = self.validation_func(
            X_history, x_current, adj_matrix, G, idx)
    ...
    def configure_optimizers(self):
        """Set the initial learning rates for different sets of parameters."""
        modules = {
